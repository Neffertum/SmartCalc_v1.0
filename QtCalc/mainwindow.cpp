...
void MainWindow::my_magic() {
  const char *my_str = ui->result_show->text().toLocal8Bit().data();
  double value_x = 0.0;
  ...
  if (!my_status) {
    ...
    double result = polish(my_str, value_x);
    if (result == NAN) {
...
void MainWindow::my_graph() {
  ...
  if (flag_x == 0) {
    ...
    if (xBegin_status == false || xEnd_status == false) {
      ...
      double yBegin = polish(my_str, xBegin);
      double yEnd = polish(my_str, xEnd);
      double temp_y = 0.0;
      QVector<double> x, y;

      for (double X = xBegin; X <= xEnd; X += h) {
        x.push_back(X);
        temp_y = polish(my_str, X);
        y.push_back(temp_y);
        if (temp_y <= yBegin) {
          yBegin = temp_y;
        } else if (temp_y >= yEnd) {
...
void MainWindow::credit() {
  ...
  if (rate_status == false || amount_status == false || years_status == false) {
    ...
    if (button->text() == "CALCULATE THE ANNUITY CREDIT!") {
      ...
      ui->c_mpay->setText(QString::number(mpay));
      ui->c_opay->setText(QString::number(opay));
      ui->c_tpay->setText(QString::number(tpay));
